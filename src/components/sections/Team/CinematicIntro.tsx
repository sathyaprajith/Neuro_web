...
export function CinematicIntro() {
  ...
  const sideX = isDesktop ? Math.min(viewportWidth * 0.26, 400) : 0;
  const cardWidth = isDesktop
    ? "w-[min(42vw,52vh,560px)]"
    : "w-[min(76vw,60vh,300px)]";
  const founderLift = isDesktop ? 0 : -Math.min(viewportHeight * 0.09, 64);
  const headDrop = isDesktop ? 0 : Math.min(viewportHeight * 0.1, 72);
  ...
  const hold = 1 - endFade;
  const pre = 1 - endFade * 2;
  ...
  useMotionValueEvent(founderOpacity, "change", (value) => {
    if (founderLayerRef.current) founderLayerRef.current.style.pointerEvents = value > 0.5 ? "auto" : "none";
  });
  useMotionValueEvent(headOpacity, "change", (value) => {
    if (headLayerRef.current) headLayerRef.current.style.pointerEvents = value > 0.5 ? "auto" : "none";
  });

  return (
    <div ref={stageRef} className="relative h-[280svh] md:h-[340svh]">
      <div className="sticky top-0 flex h-svh items-center justify-center overflow-hidden">
        <div aria-hidden className="absolute inset-0" style={{ background: "radial-gradient(52% 44% at 32% 34%, var(--glow-a), transparent 70%), radial-gradient(46% 40% at 70% 66%, var(--glow-b), transparent 70%)" }} />

        <motion.div aria-hidden className="absolute inset-0 grid place-items-center" style={{ opacity: ringsOpacity, scale: ringsScale }}>
          <motion.svg viewBox="0 0 100 100" fill="none" className="h-[135vmin] w-[135vmin] text-coral" animate={{ rotate: 360 }} transition={{ duration: 140, repeat: Infinity, ease: "linear" }}>
        ...
        {isDesktop ? (
          <motion.div aria-hidden className="absolute inset-0 z-10 grid place-items-center" style={{ opacity: lineOpacity }}>
            <motion.span className="block h-px w-[min(24vw,320px)] rounded-full bg-gradient-to-r from-coral via-plum to-sage shadow-[0_0_18px_var(--glow-a)]" style={{ scaleX: lineDraw }} />
          </motion.div>
        ) : null}

        <motion.div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center px-5" style={{ opacity: headerOpacity, y: headerY, scale: headerScale, filter: headerBlur }}>
          <SectionHeader align="center" id="team-title" eyebrow="05 · The People" title={<>The minds behind <em className="not-italic text-coral">Neuro Paradigm</em>.</>} lede="One founder's question became a structure — and the structure became twenty minds working where neuroscience meets software." />
        </motion.div>

        <div ref={founderLayerRef} className="pointer-events-none absolute inset-0 z-20 grid place-items-center">
          <motion.div className={cardWidth} style={{ y: founderY, x: founderX }}>
            <motion.div style={{ opacity: founderOpacity, scale: founderScale, filter: founderBlur, rotateX: founderRotX, transformPerspective: 1000 }}>
        ...
        <div ref={headLayerRef} className="pointer-events-none absolute inset-0 z-20 grid place-items-center">
          <motion.div className={cardWidth} style={{ y: headY, x: headX }}>
            <motion.div style={{ opacity: headOpacity, scale: headScale, filter: headBlur, rotateX: headRotX, rotateZ: headRotZ, transformPerspective: 1000 }}>
              <motion.div animate={{ y: [0, -7, 0] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut", delay: 1.2 }}>
        ...
        <div aria-hidden className="pointer-events-none absolute inset-x-0 bottom-8 z-30 grid place-items-center px-5">
          <div className="relative h-4 text-center font-mono text-[10px] tracking-[0.3em] uppercase text-ink-soft">
            <motion.span className="absolute inset-0 whitespace-nowrap" style={{ opacity: capAOpacity }}>Act 01 · Origin</motion.span>
            <motion.span className="absolute inset-0 whitespace-nowrap" style={{ opacity: capBOpacity }}>Act 02 · Structure</motion.span>
            <motion.span className="absolute inset-0 whitespace-nowrap" style={{ opacity: capCOpacity }}>Act 03 · The Collective ↓</motion.span>
          </div>
        </div>
      </div>
    </div>
